...
def getGanhoAtributo(attrib,classesPossiveis,entropiaSistema): # para cada atributo do indice pegar as possibiliades
    ...
    for linha in dataset:
        todasPossibilidades.append(linha[attrib.name])
        if (linha[attrib.name]) not in possibilidades:
            possibilidades.append(linha[attrib.name])
    ...
    for valorAtributo in possibilidades: #para cada possibilidade que o atributo pode assumir
        ...
        for linha in dataset: #para cada linha da base de dados
            if(linha[attrib.name] == valorAtributo): # se o atributo selecionado é = possibilidade
...
for i in range(len(nomesAtributos)):
    ...
    if(i != atClass):
        ...
        ganhoAtrib.update({nomesAtributos[i]:round(getGanhoAtributo(at,classesPossiveis,entropiaS),3)})
        print("Ganho de informação do atributo:",ganhoAtrib[nomesAtributos[i]])
        at.informationGain = round(getGanhoAtributo(at,classesPossiveis,entropiaS),3)
    else:
        at.isitclass = True
